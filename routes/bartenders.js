...
router.get("/", async (req, res) => {
  ...
  res.send(setResponse(bartenders));
});
...
router.get("/me", auth, async (req, res) => {
  ...
  if (!me)
    return res
      .status(400)
      .send(setResponse(false, "Invalid token provided"));
...
router.get("/:id", validateId, async (req, res) => {
  ...
  if (!bartender)
    return res.status(404).send(setResponse(false, `No bartender with this id ${req.params.id}`));
  res.send(setResponse(bartender));
});
...
router.post("/", async (req, res) => {
  ...
  if (!req.body.password) res.status(400).send(setResponse(false, '"Password" is required!'));
  ...
  //send response
  res.send(setResponse(_.pick(bartender, ["_id", "username", "email", "role"])));
});
...
router.put("/me", auth, async (req, res) => {
  ...
  if (req.body.password)
    return res.status(400).send(setResponse(false, '"Passowrd" is not allowed!'));
  //search for errors in the body of the request
  const error = new Bartender().validateBartender(req.body);
  if (error) return res.status(400).send(setResponse(false, error.details[0].message));
  // check if the email is available
  ...
  res.send(setResponse(_.pick(me, ["_id", "username", "email", "personalInfo"])));
});
...
router.put("/me/change-password", auth, async (req, res) => {
  ...
  if (!me) return res.status(400).send(setResponse(false, `Invalid token`));
...
router.put("/me/set-avatar", auth, async (req, res) => {
  ...
  if (error) return res.status(400).send(setResponse(false, error.details[0].message));
  //set the avatar
  const me = await Bartender.updateOne(
    { _id: id },
    { $set: { avatar: req.body } }
  );
...
router.put("/me/add-experience", auth, async (req, res) => {
  ...
  if (!me.n) return res.status(400).send(setResponse(false, "Invalid token proviedd"));
  res.send(setResponse(me));
});

//remove experience
router.delete(
  "/me/remove-experience/:id",
  auth,
  validateId,
  async (req, res) => {
    //set the 'me' user
    ...
    if (!me.nModified)
      return res.status(404).send(setResponse(false, "No experience with this id"));
    res.send(setResponse(me));
  }
);
...
router.delete("/me", auth, async (req, res) => {
  ...
  if (!me) return res.status(400).send(setResponse(false, "Inavalid token provided"));
  res.send(setResponse(`${me.username} user was successfully removed from the DB`));
});
