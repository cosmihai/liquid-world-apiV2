...
router.get("/me", auth, async (req, res) => {
  ...
  res.send({ success: true, message: "Data retrieved successfully", data: me });
});
...
router.get("/:id", validateId, async (req, res) => {
  ...
  if (!bartender)
    return res.status(404).send(`No bartender with this id ${req.params.id}`);
  res.send(bartender);
});
...
router.put("/me", auth, async (req, res) => {
  ...
  if (req.body.password)
    return res.status(400).send({ message: '"Passowrd" is not allowed!' });
  //search for errors in the body of the request
  const error = new Bartender().validateBartender(req.body);
  if (error) return res.status(400).send(error.details[0]);
  // check if the email is available
...
router.delete(
  ...
  async (req, res) => {
    ...
    if (!me.nModified)
      return res.status(404).send({ message: "No experience with this id" });
    res.send(me);
  }
);
...
router.delete("/me", auth, async (req, res) => {
  ...
  res.send({
    message: `${me.username} user was successfully removed from the DB`,
  });
});
