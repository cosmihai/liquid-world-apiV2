...
describe("/api/cocktails", () => {
  ...
  describe("GET /", () => {
    ...
    it("Should return the list with all cocktails", async () => {
      ...
      expect(res.body.data.some(c => c.name ==='cocktail1')).toBeTruthy();
      expect(res.body.data.some(c => c.name ==='cocktail2')).toBeTruthy();
    });
  });
  describe("GET /:id", () => {
    ...
    it("Should return 404 if no cocktail is found", async () => {
      ...
      expect(res.status).toBe(200);
      expect(res.body.success).toBe(true);
      expect(res.body.data.currentCocktail).toHaveProperty('name', 'cocktail1');
    });
  });
  describe("POST /", () => {
    let payload;
    ...
    it("Should return 200 if cocktail is valid", async () => {
      ...
      expect(res.status).toBe(200);
      expect(res.body.success).toBe(true);
      expect(res.body.data).toHaveProperty('name', 'cocktail1');
      expect(res.body.data.owner).toHaveProperty('_id', bartender._id.toString());
