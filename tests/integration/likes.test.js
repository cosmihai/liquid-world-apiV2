...
describe("/api/likes", () => {
  ...
  describe("POST /", () => {
    ...
    it("Should return 401 if logged user has restaurant role", async () => {
      const res = await exec(new Restaurant().generateToken(), {
    ...
    it("Should return 401 if logged user has bartender role", async () => {
      const res = await exec(new Bartender().generateToken(), {
        cocktailId: cocktail._id,
      });
      expect(res.status).toBe(401);
      expect(res.body.success).toBe(false);
    ...
    it("Should return 404 if no cocktail is found with the given id", async () => {
      const res = await exec(customerToken, {
        cocktailId: new mongoose.Types.ObjectId(),
      });
    ...
    it("Should add this cocktail to customer's fav cocktails list", async () => {
      ...
      expect(customer.favCocktails[0].toString()).toMatch(
        cocktail._id.toString()
      );
    });
  });
  describe("DELETE /", () => {
    ...
    it("Should return 401 if logged user has bartender role", async () => {
      const res = await execDel(new Bartender().generateToken(), {
        cocktailId: cocktail._id,
      });
      expect(res.status).toBe(401);
      expect(res.body.success).toBe(false);
    ...
    it("Should return 404 if no cocktail is found with the given id", async () => {
      const res = await execDel(customerToken, {
        cocktailId: new mongoose.Types.ObjectId(),
      });
      expect(res.status).toBe(404);
      expect(res.body.success).toBe(false);
    ...
    it("Should return 400 if the cocktail is not in fav cocktails list", async () => {
      ...
      expect(res.body.success).toBe(false);
      expect(res.body.message).toMatch(
        "This cocktail is not in your favorite list"
      );
    });
